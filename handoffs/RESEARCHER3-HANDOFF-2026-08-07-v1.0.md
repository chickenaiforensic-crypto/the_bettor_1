...
## 7. v2.1 correction (2026-08-07)

Per Director defect notice against commit a3b73f4:
- Fixed MATCH field order on all 117 v2.0 rows. They now read
  `MATCH|date|competition|compType|home|hg|ag|away|venue|stadium|city|country|tieId|sourceId`
  with empty `tieId` (position 13) and source label in `sourceId` (position 14).
- Added the 21 in-scope 2024 US Open Cup rows that v2.0 had wrongly omitted under
  the assumption that they were already in the store. The clean import target
  contains zero USOC rows, so they belong in the pack. Scores and identities
  taken from the existing verified `make_pack_usa.py` ledger (which carries the
  AET-corrected 90-minute scorelines) and cross-checked against
  rsssf/tables `usa2024.txt` on the GitHub mirror.
- Added the `Loudoun United FC` TEAM declaration (LAFC 3-0 Loudoun, R16).
- v2.1 totals: 138 MATCH (2022:28, 2023:30, 2024:21, 2025:31, 2026:28),
  23 TEAM, 20 SOURCE, one `END`. Every MATCH row has exactly 14 pipe-delimited
  fields with an empty tieId and a non-empty sourceId.
