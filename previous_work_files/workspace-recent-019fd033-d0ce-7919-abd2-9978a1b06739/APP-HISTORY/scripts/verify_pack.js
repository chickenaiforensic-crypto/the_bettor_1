...
S.document.getElementById("bpImportText").value = fs.readFileSync("/home/user/packs/czech-team-pack.txt","utf8");
...
const out = evX(`(function(){
  ...
  var ids=Object.keys(st.identities).filter(function(k){return /liberec|teplice/i.test(k);});
  return {ids:ids,
    ev:(function(){var lib=ids.filter(function(k){return /liberec/i.test(k)})[0], tep=ids.filter(function(k){return /teplice/i.test(k)})[0];
