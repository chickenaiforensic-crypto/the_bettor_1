/* audit_fixtures.js — v2.8.9 engine audits: FC Zurich v Servette (Swiss SL R2, 2026-08-01)
...
const SUI = JSON.parse(fs.readFileSync("/home/user/swiss/swiss_universe.json", "utf8"));
const SNAMES = JSON.parse(fs.readFileSync("/home/user/swiss/swiss_names.json", "utf8"));
const st = S.BlueprintEmbed.store();
[...new Set(RPL.concat(SUI).flatMap(m => [m.home, m.away]))].forEach(s => {
  if (!st.identities[s]) st.identities[s] = { id: s, name: NAMES[s] || SNAMES[s] || s };
});
...
SUI.forEach(m => st.matches.push({ id: [m.date, m.comp, m.home, m.away, m.hg, m.ag, "home"].join("|"), date: m.date, competition: m.comp, homeId: m.home, awayId: m.away, hg: m.hg, ag: m.ag, venue: "home" }));
function audit(label, homeId, awayId, homeName, awayName, cutoff) {
...
audit("FC Zurich v Servette — Swiss Super League R2 — 2026-08-01 (Letzigrund)", "zurich", "servette", "Zurich", "Servette", "2026-08-01");
audit("Dynamo Makhachkala v Lokomotiv Moscow — RPL MD2 — settled 2-1 (Makhachkala home)", "dinamo-mkh", "lokomotiv", "Makhachkala", "Lokomotiv", "2026-08-02");
audit("Lokomotiv Moscow v Dynamo Moscow — RPL listed 2026-08-29 — evidence as of 2026-08-01", "lokomotiv", "dinamo-msk", "Lokomotiv", "Dynamo M", "2026-08-29");
