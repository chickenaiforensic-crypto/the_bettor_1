...
> **⚠ AMENDMENT 2 in force (2026-08-02, owner decree D12):** the Requests module is a **CENTRAL** system — one snapshot of everything + one central request covering the entire system + central return to the same intake. Per-league request *files* are abolished; per-league scope survives only as sections inside the one request. Full spec: `AMENDMENT-2-CENTRAL-REQUEST.md`. Anything below that conflicts is superseded.
