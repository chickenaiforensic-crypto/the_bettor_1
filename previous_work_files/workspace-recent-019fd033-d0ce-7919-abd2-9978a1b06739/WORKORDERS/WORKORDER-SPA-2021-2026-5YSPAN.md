# WORK ORDER -- Spain La Liga 5-year-span 2021-2026 up-to-today (researcher commission WO-SPA-SPAN-13)

**Issued:** 2026-08-03 - **Status:** STAGED -- queue position 13 - research may run in parallel (owner decree 2026-08-02); auditor approvals remain one card per return in queue order - **Format of return:** ONE `BP-TEAM-PACK v2` block (text file `.txt` -- never zip, never paste fragments)
...
This is **Spain La Liga** (Spain). **Before returning anything, scan your own rows: any club outside the section-3 roster = wrong competition -- stop.** (The first return attempt in this programme arrived as the wrong country; that failure is on record.)
...
|---|---|---|
| Spain La Liga | 2021-22, 2022-23, 2023-24, 2024-25, 2025-26 | 380 per season = **1,900** |
| Spain La Liga | 2026-27 through your return date | state last round/date in a NOTE |

20 clubs, 38 matchdays, every club exactly 38 matches -- steady all five seasons.
**Not in this order:** Copa del Rey, Europe, Segunda division.
...
- `MATCH|<dateISO>|<competition>|<compType>|<home>|<hg>|<ag>|<away>|<venue>|<stadium>|<city>|<country>||<sourceLabel>`
  - `<competition>` string, verbatim: `Spain La Liga` (declare it once in a `NOTE|info|catalog`)
...
- `TEAM|<name>|Spain||SPA|...` -- **NOT expected:** every club of the 2021-26 window is already on our roster (section 3). If you believe one is missing, stop and write `NOTE|warning|blocker`; do NOT invent an identity.
...
**Rename/spelling traps (map silently to the roster strings, NOTE each rule once):** Ath Bilbao / Ath Madrid keep these forms (never Athletic/Atletico) - Vallecano (never Rayo) - Sp Gijon - Sociedad and Betis stay short.

## 4. SOURCE HIERARCHY + VERIFICATION (non-negotiable)

1. **RSSSF round-by-round + final tables = primary:** `rsssf.org/tabless/span<YEAR>.html` (2021-22 season = the `...2022.html` page, through 2025-26 = `...2026.html`; 2026-27 = `...2027.html`).
...
Save as `SPA-2021-2026_BP-TEAM-PACK_v2.txt`, hand to the owner (repo folder `handoffs/` -- or chat as .txt). Auditor drill: recompute all five tables --> boundary/dedupe scan --> span-diff --> one staged approval card --> commit through the app's own intake.
