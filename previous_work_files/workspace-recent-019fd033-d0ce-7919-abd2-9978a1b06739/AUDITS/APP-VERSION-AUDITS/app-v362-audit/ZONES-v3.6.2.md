...
## Z-2026-08-02-013 — WO-SCOPE-CLEAR-01: Country-Scope Clear Control (v3.6.0)

- **Decision:** Ship `app/app-v3.6.0.html` — version **v3.6.0** (badge + footer), md5 `edf52d78b2fa1690721aa3a72018b634`, 630,363 B, backup `backups/app-v3.6.0-edf52d78.html`, deliver `deliver/8/`. v3.5.2 stays sealed.
- **Owner decree (WO-SCOPE-CLEAR-01, "no mixing, absolute"):** a per-country UI control to clear old/incorrect scope data so corrected packs load as pure adds — no per-country code, ever. Registered as D15. This is the sanctioned hard-clear path: MUTE stays the soft/reversible path (rows visible, excluded), PURGE is backup-gated (auto-downloads the existing-format export first; undo = load that backup via the migration feature), atomic, idempotent, fully audited (pre/post entries + log lines per operation).
- **Built:** `app/modules/scope.js` (store-derived scopes; `selection` = single source of truth for preview/confirm/mutation; `muteScope`/`unmuteScope`/`purgeScope`), Data-tab "Country packs — clear & replace" panel (list → preview with full row list + orphan/cross-border keep split → MUTE/UNMUTE or backup-gated PURGE → confirm), cross-border guard, orphan rule, sources kept by default (optional drop), replace handoff to the file drop.
- **Only shared-code touch:** derive Coverage now counts non-muted rows (muted convention, required by G4) — RPL coverage 644→641 on seeds (3 mutes held out), no other numbers change.
- **UAT numbers reproduced exactly on the owner's real export:** Russia 644 (489/152/2/1, 26 clubs) · Czech 632 (561/63/8, 45 clubs) · US 81 · Scotland 34 · Kosovo 19.
- **Evidence:** `gate-evidence/scope-clear.md`; `harness/acceptance-scope.js` **33/33** (G1–G12); suites: smoke 49/49 · R8 13/13 · R9 7/7 · R10 12/12 · R11 18/18 · parity 7/7 · legacy 156/156 · CF grep 0.
- **Status:** ✅ shipped — owner UAT per WO §5 in order; auditor records the seal.


---

## Z-2026-08-02-014 — R2 fix: scope preview shows the FULL row list (v3.6.1)

- **Decision:** Ship `app/app-v3.6.1.html` — version **v3.6.1** (badge + footer), md5 `762a62846eb5c9531627e1d67be365a8`, 630,593 B, backup `backups/app-v3.6.1-762a6284.html`, deliver `deliver/9/`. v3.6.0 stays sealed (md5 `edf52d78…`).
- **Fix:** the scope preview's scrollable row list was capped at the first 400 rows (`slice(0, 400)`); WO-SCOPE-CLEAR-01 requires the **full** row list. Now renders every in-scope match (Russia = all 644). The `.scope-rows` container already scrolls, so large scopes are fully browsable.
- **Pin:** `harness/acceptance-scope.js` G2 UI render — preview view renders **all 644** Russia rows (count of `class="srow"` = 644). Scope suite 34/34.
- **Evidence:** `gate-evidence/scope-clear.md` (R2 note); suites on the final file: smoke 49/49 · R8 13/13 · R9 7/7 · R10 12/12 · R11 18/18 · scope 34/34 · parity 7/7 · legacy 156/156 · CF grep 0 · fitted-fitted grep 0.
- **Status:** ✅ shipped · **Rollback:** `backups/app-v3.6.1-762a6284.html` + legacy `reference/app-v2.6-cross.html`.


---

## Z-2026-08-02-015 — WO-SCOPE-CLEAR-01 v3.6.1 delta (D1 + D2 + D3), shipped as v3.6.2

- **Decision:** Ship `app/app-v3.6.2.html` — version **v3.6.2** (badge + footer), md5 `c7f955d4aacdeaaca9a44e4314f2b14e`, 634,591 B, backup `backups/app-v3.6.2-c7f955d4.html`, deliver `deliver/10/`. The amended workorder's v3.6.1 delta = D1 + D2 + D3; D2 (full preview list) already shipped in v3.6.1 (`762a6284…`); per the standing version policy (upward, no reused names, sealed versions untouched) the complete three-item delta ships as **v3.6.2**. v3.6.0 and v3.6.1 stay sealed.
- **D1 (league-level clear):** two-level hierarchy country → competitions; both levels selectable; selection generalised to `{country, competition|null}` through ONE shared `selection()` (preview/confirm/mute/purge); orphan rule operates on the narrowed set. Gate G2-L: MOL Cup alone → 63; PURGE → 1,432−63 = 1,369; Czech First League (561) + playoffs (8) untouched and still render; 0 casualties outside the 63.
- **D2 (full list):** preview renders ALL in-scope rows (no 400 cap) — re-verified (644/644 Russia rows).
- **D3 (alphabetical):** country list A–Z, competitions inside a country A–Z, removed/kept club lists A–Z (counts stay next to names; ordering only). Gate G13: Russia competitions read exactly Russian Cup · Russian Premier League · Russian Relegation Playoffs · Russian Super Cup (UI + selection pins).
- **G10 hygiene:** removed the example competition literal from a comment so the no-hardcoding grep is clean.
- **Evidence:** `gate-evidence/scope-clear-delta.md`; `harness/acceptance-scope.js` **43/43**. Suites: smoke 49/49 · R8 13/13 · R9 7/7 · R10 12/12 · R11 18/18 · parity 7/7 · legacy 156/156 · CF grep 0.
- **Status:** ✅ shipped · **Rollback:** `backups/app-v3.6.2-c7f955d4.html` + legacy `reference/app-v2.6-cross.html`.
