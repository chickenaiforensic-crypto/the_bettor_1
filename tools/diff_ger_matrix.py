#!/usr/bin/env python3
"""Score-level diff: GER 2025-26 league carrier (MD-rows, openfootball under
source_adaptation) vs the independent second-index FBR matrix (MX-rows, Wikipedia
2025-26_Bundesliga results matrix). Mirrors tools/diff_epl_matrix.py.
...
The MD prefix is accepted alongside R so the 2025-26 carrier file can be compared directly.

Usage: python3 tools/diff_ger_matrix.py audit/ledger/ger-2ndidx-2025-26.txt audit/ledger/ger-2ndidx-2025-26-MX.txt
"""
import sys
import re
...
def main():
    ...
    print(f"carrier rows: {len(primary)} ({primary_path}) goals={goals_primary}")
