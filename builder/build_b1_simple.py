...
assert base_md5 == "e688eee2d0fe4009b60cab253335eceb", f"baseline {base_md5}"
src = BASE.read_text(encoding="utf-8")

# A version bump
src = src.replace("var APP_VERSION = '3.7.0'; /* B0:", "var APP_VERSION = '3.8.0'; /* B1: S1 LIVE-DERIVE-01 live re-derive + auto re-validation + provenance panel M3 + live form stars G17 + retire __DC_GATE__ + teamStats fix M6 + compliance map M18 + EPL live revalidation G16 + legacy flags M4 + B0:", 1)

# B CSS provenance panel
CSS = """.provenance-panel{border:1px dashed color-mix(in srgb,var(--accent) 35%,transparent);border-radius:8px;padding:8px 12px;margin:8px 0;background:color-mix(in srgb,var(--card) 85%,transparent);font-size:11.5px;line-height:1.5}
...
"""
src = src.replace(".replay-report .ok{color:var(--accent);font-weight:600;margin-top:6px}\n", ".replay-report .ok{color:var(--accent);font-weight:600;margin-top:6px}\n"+CSS, 1)

# C LiveTeamRecord + liveStarsFor + autoRevalidate + provenance panel + teamStats + EPL live revalidation + compliance map + retire __DC_GATE__ text

INJECT = """
  /* ---- B1 S1 G17 live form stars from store + M3 provenance panel + M6 teamStats fix + G16 EPL live revalidation + M18 compliance map + G14 M1 auto re-validate + M4 legacy flags drop ---- */
  ...
  function isSufficientForLiveFit(store, leagueKey){
    ...
    store.matches.forEach(function(m){
      ...
      if(compC===codeC){ var s=PR.derive.seasonOf(m.dateISO); seasons[s]=(seasons[s]||0)+1; }
    });
...
# Insert before function starsFor
src = src.replace("  function starsFor(store, code, teamName) {", INJECT + "\n  function starsFor(store, code, teamName) {\n    var live=liveStarsFor(store,code,teamName);\n    if(live!==null) return live;\n")

# Fix predictOnline starsHome null -> live
src = src.replace("      lh: lh, la: la, H: H, D: D, A: A, starsHome: null, starsAway: null, starAdj: false,",
...
src = src.replace("    var prov = res.provenance && res.provenance.online\n      ? '<div class=\"provenance dim\">' + C.esc(res.provenance.online) + '</div>'\n      : '';",
                  "    var prov = res.provenance && res.provenance.online\n      ? '<div class=\"provenance dim\">' + C.esc(res.provenance.online) + '</div>'\n      : '';\n    prov += renderProvenancePanel(store);")

# Auto re-validate after pack commit — global replace STORE.save + PR.derive.invalidate pattern
src = src.replace("    STORE.log(store, { type: 'data', action: 'commit', summary: 'Pack committed: ' + report.matches + ' matches, ' + report.teams + ' teams, ' + report.seasons + ' season rows, ' + report.mutes + ' mutes.', report: report });\n      }\n\n      if (opts.onCommitted) opts.onCommitted(store);",
                  "    STORE.log(store, { type: 'data', action: 'commit', summary: 'Pack committed: ' + report.matches + ' matches, ' + report.teams + ' teams, ' + report.seasons + ' season rows, ' + report.mutes + ' mutes.', report: report });\n      }\n      try{ autoRevalidate(store); }catch(e){}\n\n      if (opts.onCommitted) opts.onCommitted(store);")

# Retire __DC_GATE__ to provenance text (both occurrences, replace data assignment with note)
src = src.replace(
