...
**v1.2 amendment (A-08 registered as PROPOSED):** owner cross-league doctrine question (2026-08-05) — evidence audit appended (old tree holds ten v2.9.x "-cross" builds with an UNWEIGHTED bridge, scale 1.00; zero cross-league code in v3.6.3; star scales proven per-league via old `star_cutoffs.json`), amendment A-08 opened (pending owner), ledger M19 added. v1.1 md5 `5dc8df75aa60cc7705c24ba8bda543ff` (superseded, kept in ZONES).

**v1.3 (READY-STATE for planner + auditor, 2026-08-05):** owner ruling applied — A-03/A-04/A-06/A-08 **APPROVED for documentation/planning only** (not a build order; every build still needs an explicit owner go-signal per P5). Closing census executed on the fresh backup (`Supervior/other/pitch-rating-full.json` @ repo sha `12192a9b…`, sha256 `c7b29e85…8fc00`): store **5,000 matches exact**, fully reconciled to the owner's own app log (55 entries); small-country set 156 + leftovers 4 purged by owner decision (in-app backup-gated each); one open data defect — M20 (MOLCUP old file imported, −82 rows, fix pinned). §14 RESOURCES table added: this table is now the **live pin authority**; any md5 quoted elsewhere in this document is superseded by §14. Ledger now M1–M20. v1.2 md5 `a4ace3ceab3eda062bb547374599aecf` (superseded, kept in ZONES).
...
|---|---|---|---|---|
| K1 | **THIS DOCUMENT (SOT v1.3)** | the map: systems, status, doctrine, ledger | root | (self-pinned in ZONES) |
...
| M19 | Cross-league WEIGHTED bridge (league-strength ratings across leagues — e.g. an EPL side vs Dynamo Moskva rated on a weighted common scale, then standard computation applies) | owner doctrine 2026-08-05; blueprint Study 17 (proposed, never approved); v2.9.x builds ran an UNWEIGHTED bridge (scale 1.00) dropped at the v3 line | omitted from v3.6.x — zero code paths (grep-proven) | DECISION → **APPROVED-FOR-DOCUMENTATION (A-08)** | A-08 frozen plan: UEFA 2021-26 connector pack → LIVE-DERIVE-01 replay validation vs frozen 1.00 baseline → v3.7.x |
| M20 | MOLCUP fullspan shortfall: owner imported OLD `MOLCUP-2021-2026.txt` (120 rows) instead of ADOPTED `MOLCUP-FULLSPAN.txt` (202) — committed set byte-matches old file exactly; 82 rows missing (49×2025, 26×2024, 7×2026) | census 2026-08-05 on fresh backup | open data defect; everything else pin-exact | YES (data fix, no code) | owner imports FULLSPAN file (dedupe add-if-new → +82, skips 120); toast must read "Loaded — 82 matches."; approve any Z-003 held cards via the verbatim-approve button; final store **5,082** (ENG 1,900 · CZE 1,603 · RUS 1,579) |
...
# 14. RESOURCES — live pin authority (v1.3)

Documents point to resources here. Repo = `github.com/chickenaiforensic-crypto/the_bettor_1`, verified at branch `main` tip **`12192a9be8213b5ce8210e66396106d602237857` (2026-08-05T06:04:16Z)** — 965+2 files, ~293 MB. When the repo moves, re-pin this row before citing it.

| Resource | Where | Pin |
|---|---|---|
| Fresh store backup (closing census input, 5,000 matches, exported 2026-08-05T06:03:26Z) | repo `main` : `Supervior/other/pitch-rating-full.json` | sha256 `c7b29e8501319b8024cc7b2d11a1d2309248e5edcb4a87751484ed94e8d8fc00` · md5 `0500b8b2e7f188d0c177a2db7c65960e` |
| Old project tree (trainer, ratings v1/v2, star data, chain edges, exports, 136 .py) | repo `main` : `previous_work_files/workspace-019fb2c3-…/` | commit-pinned above (OLD-PORT-01 input; closes M12; feeds M18) |
| App history incl. ten v2.9.x "-cross" builds | repo `main` : `previous_work_files/workspace-recent-019fd033-…/APP-HISTORY/` | commit-pinned above |
| RPL/MOL audit workdirs (sweeps, replays) | repo `main` : `previous_work_files/workspace-recent-019fd033-…/AUDITS/COUNTRY-WORKDIRS/` | commit-pinned above |
| Owner workorders (5YSPAN series, RPL backfill) + updates | repo `main` : `Supervior/` | commit-pinned above |
| ENGINE_SPEC.md (Dixon-Coles spec) | Arena workspace `uploads/` | md5 `91cd0cd5420cd494a799bd4050cb2ef8` |
| LIVE-BLUEPRINT.md (evidence engine) | Arena workspace `uploads/` | md5 `d01cfde0b7e75f62646bb20eb470233a` |
| METHODOLOGY.md (principles/protocols/error register) | Arena workspace `uploads/` | md5 `6cd6c0c8ebc695a8fe3afc313ddc90ac` |
| Current app v3.6.3 (ADOPTED) + README-UAT | Arena workspace `APP-V3.6.3/` | md5 `17dd2b5b66ceb572a3fd946db9b56a92` · sha256 `268dc5296189cf3016847624ba180cb14904a35a07bb2648428581bb78dad0f9` |
| Audit card + six ADOPTED packs + gates | Arena workspace `AUDITS/AUDIT-OVERRIDE-2026-08-04/` | card md5 `b9e1775eb56128978b88efef4af876cb` (+ADDENDUM-3); MOLCUP FULLSPAN `f2ee00065ba8a8e655003ee77fb618ff` (202 rows — the correct file; the 120-row `MOLCUP-2021-2026.txt` md5 `662fe5dfe38002474855110b2a17ea6c` is SUPERSEDED, history only) |
| Baseline store (pre-programme reference) | Arena workspace `REFERENCE/pitch-rating-full-data-2026-08-02.json` | md5 `5a8ba49475acfa2340ce7fd66e4dfeb0` |
| Ops chain (every event, chronological) | Arena workspace `ZONES.md` | current v0.90 md5 — see ZONES footer |
| Outbox (builder/researcher packets) | Arena workspace `OWNER-OUTBOX/` + `INDEX.txt` | see INDEX (DELIVER-06 READY TO SEND; next number DELIVER-07) |

**Role entries for planner + auditor:** planner works from **§10 ledger (M1–M20)** + **§12 amendments register** — every row carries its route-in; auditor works from **§14 pins** + ZONES verification protocol (grep-verify on the actual file, md5/sha on arrival, harness shown never claimed, no silent rewrites, errata owned). Nothing in this document is asserted without a doc section, code line, or pinned file behind it.
